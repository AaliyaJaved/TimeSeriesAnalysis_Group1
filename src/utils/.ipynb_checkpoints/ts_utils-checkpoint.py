...
def _remove_nan_union(array_a, array_b):
    """
    Returns the two input arrays where all elements are deleted that have an index 
    that corresponds to a NaN value in either of the two input arrays.
    """
    # Ensure the arrays are 1-dimensional
    array_a = np.ravel(array_a)
    array_b = np.ravel(array_b)

    # Create a boolean mask where either array contains NaN values
    isnan_mask = np.logical_or(np.isnan(array_a), np.isnan(array_b))

    # Use np.delete to remove NaNs from both arrays
